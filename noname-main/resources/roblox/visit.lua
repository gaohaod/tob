...
game:GetService("ScriptInformationProvider"):SetAssetUrl("http://noname.xyz/Asset/")
game:GetService("ContentProvider"):SetThreadPool(16)
pcall(function() game:GetService("InsertService"):SetFreeModelUrl("http://noname.xyz/Game/Tools/InsertAsset.ashx?type=fm&q=%s&pg=%d&rs=%d") end) -- Used for free model search (insert tool)
pcall(function() game:GetService("InsertService"):SetFreeDecalUrl("http://noname.xyz/Game/Tools/InsertAsset.ashx?type=fd&q=%s&pg=%d&rs=%d") end) -- Used for free decal search (insert tool)

ifSeleniumThenSetCookie("SeleniumTest2", "Set URL service")

settings().Diagnostics:LegacyScriptMode()

game:GetService("InsertService"):SetBaseSetsUrl("http://noname.xyz/Game/Tools/InsertAsset.ashx?nsets=10&type=base")
game:GetService("InsertService"):SetUserSetsUrl("http://noname.xyz/Game/Tools/InsertAsset.ashx?nsets=20&type=user&userid=%d")
game:GetService("InsertService"):SetCollectionUrl("http://noname.xyz/Game/Tools/InsertAsset.ashx?sid=%d")
game:GetService("InsertService"):SetAssetUrl("http://noname.xyz/Asset/?id=%d")
game:GetService("InsertService"):SetAssetVersionUrl("http://noname.xyz/Asset/?assetversionid=%d")
...
pcall(function() game:GetService("MarketplaceService"):SetProductInfoUrl("https://noname.xyz/marketplace/productinfo?assetId=%d") end)
pcall(function() game:GetService("MarketplaceService"):SetDevProductInfoUrl("https://noname.xyz/marketplace/productDetails?productId=%d") end)
pcall(function() game:GetService("MarketplaceService"):SetPlayerOwnsAssetUrl("https://noname.xyz/ownership/hasasset?userId=%d&assetId=%d") end)
pcall(function() game:SetCreatorID({{id}}, Enum.CreatorType.User) end)
...
function registerPlay(key)
	if true and game:GetService("CookiesService"):GetCookieValue(key) == "" then
		game:GetService("CookiesService"):SetCookieValue(key, "{ \"userId\" : {{id}}, \"placeId\" : 0, \"os\" : \"" .. settings().Diagnostics.OsPlatform .. "\"}")
	end
end
...
pcall(function() game:GetService("Players"):SetBuildUserPermissionsUrl("http://www.noname.xyz/Game/BuildActionPermissionCheck.ashx?assetId=0&userId=%d&isSolo=true") end)
...
function doVisit()
	...
	player.CharacterAppearance = "http://noname.xyz/char/{{id}}?time={{time}}?rcc=false"
	...
	if not inStudio and false then
		...
		visit:SetPing("http://noname.xyz/Game/ClientPresence.ashx?version=old&PlaceID=0", 120)
...
if success then
	...
	if not inStudio then
		if false then
			game:HttpPost("https://data.noname.xyz/Error/Lua.ashx", "Visit.lua: " .. err)
		end
	end
end
